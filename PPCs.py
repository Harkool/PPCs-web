...
if model_type == "Preoperative only":
    ...
    anemia = st.sidebar.selectbox("Preop anemia", ['No', 'Yes'])
    infection = st.sidebar.selectbox("Respiratory tract infection", ['No', 'Yes'])
    spo2 = st.sidebar.selectbox("Preop SpO2", ['≥96%', '<96%'])
    ...
    diabetes = st.sidebar.selectbox("Diabetes mellitus", ['No', 'Yes'])
    stroke = st.sidebar.selectbox("Unexpected stroke", ['No', 'Yes'])
    smoking = st.sidebar.selectbox("Smoking status", ['Never', 'Former', 'Current'])
    infection = st.sidebar.selectbox("Respiratory tract infection", ['No', 'Yes'])
    spo2 = st.sidebar.selectbox("Preop SpO2", ['≥96%', '<96%'])
    asa = st.sidebar.selectbox("ASA status", ['Ⅰ/Ⅱ', 'Ⅲ/Ⅳ/Ⅴ'])
